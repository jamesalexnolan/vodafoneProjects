...
function vt519_CLOPChanges() {
    ...
    if (totalvt519MonthsDifference === 0 || totalvt519MonthsDifference === 1) {
        document.querySelectorAll('[data-component-name="Paragraph"]')[1].innerHTML = 'There\'s <strong>£' + window.CRO_SIMPLIFIED_BASKET_CLOP_DATA.currentLoan.monthlyCost + '</strong> left to pay and ' + vt519differenceInDays + ' days remaining on your ' + window.CRO_SIMPLIFIED_BASKET_CLOP_DATA.currentLoan.deviceName + ' plan. Agreement ends: ' + window.CRO_SIMPLIFIED_BASKET_CLOP_DATA.currentLoan.agreementEndDate
    } else {
        document.querySelectorAll('[data-component-name="Paragraph"]')[1].innerHTML = 'There\'s <strong>£' + window.CRO_SIMPLIFIED_BASKET_CLOP_DATA.currentLoan.monthlyCost + '</strong> left to pay and ' + totalvt519MonthsDifference + ' months remaining on your ' + window.CRO_SIMPLIFIED_BASKET_CLOP_DATA.currentLoan.deviceName + ' plan. Agreement ends: ' + window.CRO_SIMPLIFIED_BASKET_CLOP_DATA.currentLoan.agreementEndDate
    }
    //option 1 CTA text
    ...
    //option 2 sub-header
    document.querySelectorAll('[data-component-name="Paragraph"]')[3].innerHTML = 'You’ll keep paying off this phone alongside your new one, so you’ll pay <strong>£' + window.CRO_SIMPLIFIED_BASKET_CLOP_DATA.currentLoan.outstandingBalance + '</strong> more a month.'
    ...
    //change text
    document.querySelector('[data-component-name="upgrade-plan-info"]>ul>li').innerHTML = '<span aria-hidden="true" size="2" data-component-name="Icon" class="Iconstyle__Icon-sc-yxw4v5-0 gohqvj"><span><span><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 192" class="injected-svg" data-src="/upgrade-and-offers/assets/icons/SourceStateIcons/info.svg" xmlns:xlink="http://www.w3.org/1999/xlink"><path fill="#005ea5" d="M95.75 16.25a80 80 0 1 0 80 80 80 80 0 0 0-80-80M94 55.75a5 5 0 1 1-5 5 5 5 0 0 1 5-5m10 80H88a4 4 0 0 1 0-8h4v-40h-4a4 4 0 0 1 0-8h8a4 4 0 0 1 4 4v44h4a4 4 0 0 1 0 8"></path></svg></span></span></span><b style="display: contents;">Please note:</b> Getting a new phone is subject to a credit check. Having an outstanding balance on one or more existing phones might affect this credit check.'
    //desktop
    if (screen.width > 880) {
